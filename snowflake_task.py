...
select
            ...
            REGEXP_REPLACE(TRIM(REGEXP_REPLACE(base.source_type, '[\r\n]+', ' ')), '\\s+', ' ') AS BU,
            ...
            REGEXP_REPLACE(TRIM(REGEXP_REPLACE(COALESCE(DIV0(SUM(GMV),45),0), '[\r\n]+', ' ')), '\\s+', ' ') As ORDERS_COUPONS,
            REGEXP_REPLACE(TRIM(REGEXP_REPLACE(COALESCE(DIV0(Sum(trendfam_earnings),45),0), '[\r\n]+', ' ')), '\\s+', ' ') As ORDERS_LINKS,
